...
class ProductRepositoryImpl @Inject()(typeProductRepository: TypeProductRepository)
  ...
  implicit val putTypeProduct: Put[TypeProduct] = Put[Int].tcontramap(n => n.id_typeProduct)
  ...
  val startDataTable: Unit = {
    ...
    val l = PoolConnection.transactor.use((truncatedTable, startTable).mapN(_ + _).transact[IO]).unsafeRunSync()
  ...
  override def list(): List[ProductData] = {
    ...
    PoolConnection.transactor.use(productList.transact[IO]).unsafeRunSync()
  }
  ...
  override def get(id: Int): Option[ProductData] = {
    ...
    PoolConnection.transactor.use(productDB.transact[IO]).unsafeRunSync()
  }
  ...
  override def insert(data: ProductData): Report = {
    ...
    val insert =
      sql"""insert into product (id_product, id_typeProduct, name, gender, size, price) values
        ...
        .update.run
    try {
      val n = PoolConnection.transactor.use(insert.transact[IO]).unsafeRunSync()
      n match {
  ...
  override def insert(dataList: List[ProductData]): Report = {
    ...
    val result = (for {
      ...
      val success = PoolConnection.transactor.use(toQuery(data).transact[IO]).unsafeRunSync()
      (success, data.id, if (success == 1) "Success" else "undefined error")
    } catch {
      case e: java.sql.SQLException => (0, data.id, e.getSQLState.toInt match {
        case 23505 => "ERROR: llave duplicada viola restricción de unicidad «product_pkey»  " +
          "Detail: Ya existe la llave. " + "\\ Error Code: " + e.getSQLState
        case _ => e.getMessage + " \\ Error Code: " + e.getSQLState
      })
  ...
  override def update(data: ProductData): Report = {
    ...
    try {
      val n = PoolConnection.transactor.use(updateRow.transact[IO]).unsafeRunSync()
      n match {
  ...
  override def delete(id: Int): Report = {
    ...
    try {
      val n = PoolConnection.transactor.use(deleted.transact[IO]).unsafeRunSync()
      n match {
